...
(function() {
	...
	function onscreen () {
		if ( (new Date()).getTime() - onscreen.lastEvent > 500) return stop();
		...
		if (objs.length === 0) {
			//nothing to onscreen
			stop();
			$.fn.onscreen.timeslice = 500;
			start();
		} else {
			//something to onscreen
			stop();
			$.fn.onscreen.timeslice = 250;
			start();
		}
	...
	onscreen.lastEvent = 0;
	onscreen.interval = null;

	function start() {
		lastEvent = (new Date()).getTime();
		if (onscreen.interval !== null) return;
